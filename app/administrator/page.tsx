...
export default function AdministratorPage() {
  ...
  const handleImageUpload = async (file: File, index: number): Promise<string | null> => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        showNotification(`❌ Erro ao enviar imagem: ${result.error || "Erro desconhecido"}`, "error")
        return null
      }
    } catch (error) {
  ...
  const saveAll = async () => {
    ...
    await saveMarketingManuals()

    // Save marketing warranties
    await saveMarketingWarranties()

    // Add this to the saveAll function
    const saveFactoryProduction = async () => {
  ...
  const saveMarketingContent = async () => {
    for (const content of marketingContent) {
      if (content.title_en && content.image_url) {
        try {
  ...
  const saveMarketingManuals = async () => {
  ...
  const saveMarketingWarranties = async () => {
          ...
          <div className="flex items-center gap-4">
            ...
            {(isLoading || isSavingOrder) && (
              <div className="flex items-center gap-2 text-blue-600">
                ...
                <span className="text-sm">{isSavingOrder ? "Salvando ordem..." : "Carregando..."}</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-4">
