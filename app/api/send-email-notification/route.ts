...
// Simulação de envio de email - substitua por um serviço real como Resend
...
export async function POST(request: Request) {
  try {
    ...
    if (!notificationEmail) {
      return NextResponse.json({ success: false, error: "Email de notificação não configurado" }, { status: 400 })
    }
    ...
    if (type === "order") {
      subject = `Novo Pedido - ${data.order_id}`
      html = generateOrderEmailHTML(data)
    } else if (type === "service_request") {
      subject = `Nova Solicitação de Serviço - ${data.request_id}`
      html = generateServiceRequestEmailHTML(data)
    } else {
